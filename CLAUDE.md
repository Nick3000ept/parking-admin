...
- **doPost** — `setDate` / `clearDate` c hint `{row, colDate, colSp}` для fast-path; `setEquip {num, pos, status, pct}` — статус/процент позиции монтажа оборудования. LockService только на `setValue`.
- **Монтаж оборудования в карточке**: блок в попапе у помещений из листа `Монтаж_оборудования` (load отдаёт `equip` + `equip_positions`). Права: Админ — всё; подрядчик — только помещения, где он вписан в колонку «Подрядчик» листа; Наблюдатель — просмотр. Процент редактируется только при статусе «В работе». Optimistic UI с откатом, гонка ответов защищена `equipSeq`.
- **Чекбокс «Тех помещения»** (справа от чипсов работ, рядом со «Скрыть готовые»): показывает только помещения группы панели «Тех помещения». Комбинируется с остальными фильтрами по И.
