...
| `Монтаж_оборудования` | Факт монтажа инженерного оборудования по 133 тех-помещениям. A–E справочные (номер, корпус, этаж, наименование, **Подрядчик** — заполняет админ в таблице), F–M — пары «статус + %» по 4 позициям (Поставка / Монтаж / Расключение / ПНР), N — «Обновлено» (пишется автоматически). Сайт пишет только в F–N. На сайте вводится только **процент**; статус вычисляется из него автоматически (0 — Не начато, 1–99 — В работе, 100 — Готово) и пишется в соседнюю колонку. Создан через `?action=setupEquip` (идемпотентно; повторный вызов ничего не меняет) |
...
- **doPost** — `setDate` / `clearDate` c hint `{row, colDate, colSp}` для fast-path; `setEquip {num, pos, pct}` — процент позиции монтажа оборудования (статус бэк вычисляет из процента сам). LockService только на `setValue`.
- **Монтаж оборудования в карточке**: блок в попапе у помещений из листа `Монтаж_оборудования` (load отдаёт `equip` + `equip_positions`). В блоке только поле процента (0–100); статус выводится цветом строки. Права: Админ — всё; подрядчик — только помещения, где он вписан в колонку «Подрядчик» листа; Наблюдатель — просмотр. Optimistic UI с откатом, гонка ответов защищена `equipSeq`.
